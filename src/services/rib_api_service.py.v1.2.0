...
class Modify(Rpc):
    ...
    def route(self, id=None, operation=None, table=None, key_prefix=None, key_preference=None,
              service_name=None, rtm_preference=None, metric=None, tunnel_next_hop=None, 
              follow_resolution=None,
              source_class=None, destination_class=None, json=None):
        '''
            ...
            Args:
                ...
                service_name (str): specifies VPRN; if None, applies to Base router.
                follow_resolution (bool): used in oneof next_hop.
                source_class (int): class tag for source IP matches.
                destination_class (int): class tag for destination IP matches
        ...
        if json:
            ...
            # Common Key construction for all operations
            entry_key = rib.RouteTableEntryKey(
                prefix = key_prefix,
                preference = key_preference,
                service_name = service_name # New in v1.2.0
            )

            if operation in ['add', 'replace']:
                # Constructing RouteTableEntry
                table_entry = rib.RouteTableEntry(
                                entry_key = entry_key,
                                rtm_preference = rtm_preference,
                                metric = metric,
                                source_class = source_class,      # New in v1.2.0
                                destination_class = destination_class # New in v1.2.0
                              )
            
                # Handle the oneof next_hop logic
                if follow_resolution is not None:
                    table_entry.follow_resolution = follow_resolution
                elif tunnel_next_hop is not None:
                    table_entry.tunnel_next_hop = tunnel_next_hop
                ...
                # Delete uses the updated Key (including service_name)
                if table == 'ipv4':
                    msg = rib.ModifyRequest.Request(id=id, ipv4_route_DELETE = entry_key)
                elif table == 'ipv6':
                    msg = rib.ModifyRequest.Request(id=id, ipv6_route_DELETE = entry_key)
                else:
                    raise ValueError('Invalid combination of table and operation: <{table}:{operation}>'.format(
                                                                                    table=table,
                                                                                    operation=operation))                       
