...
# --- NEW 1.2.0 OPTIONS START ---
@click.option('--service_name', default=None, help='VPRN service name; applies to Base if not specified')
@click.option('--rtm_preference', default=None, type=int)
@click.option('--metric', default=None, type=int)
@click.option('--tunnel_next_hop', default=None, help='Tunnel destination IP address')
@click.option('--follow_resolution', is_flag=True, default=None, help='Forward matching traffic according to the FIB')
@click.option('--source_class', default=None, type=int, help='Class tagged to matching source IP packets')
@click.option('--destination_class', default=None, type=int, help='Class tagged to matching destination IP packets')
# --- NEW 1.2.0 OPTIONS END ---
@click.pass_context
def route(ctx, operation, table, id, key_prefix, key_preference, service_name, 
          rtm_preference, metric, tunnel_next_hop, follow_resolution, 
          source_class, destination_class):
    
    # We call the same route method. If operation is 'delete', 
    # the service_name will now be included in the key.
    ctx.obj['manager'].rpcs[ctx.obj['RPC_TYPE']][ctx.obj['RPC_NAME']].route(
        operation=operation,
        table=table,
        id=id,
        key_prefix=key_prefix,
        key_preference=key_preference,
        service_name=service_name,      # Crucial for deleting from VPRNs
...
@click.option('--key_preference', default=None, type=int, help=('Key; this is ordering preference for '
                                                 'multiple routes with same prefix for '
                                                 'ordering within the RIB-API module'))
@click.option('--service_name', default=None) # Add this
@click.option('--ttm_preference', default=None, type=int)
@click.option('--metric', default=None, type=int)
@click.pass_context
def tunnel(ctx, operation, table, id, key_endpoint, key_preference, service_name, ttm_preference, metric):
    ctx.obj['last_req'] = ctx.obj['manager'].rpcs[ctx.obj['RPC_TYPE']][ctx.obj['RPC_NAME']].tunnel(
                                                                            ...
                                                                            service_name=service_name, # Pass this
                                                                            metric=metric
                                                                        )
